...
export class Lambertian extends Material {
  ...
  constructor(a) {
    super();
    this.albedo = a instanceof Vec3 ? new SolidColor(a) : a;
  }
   ...
   */
  scatter(rIn, rec, attenuation, scattered) {
    let scatterDirection = new Point3()
      .$add(rec.normal)
    ...
    scattered.copy(new Ray(rec.p, scatterDirection, rIn.time));
...
export class Dielectric extends Material {
  ...
  scatter(rIn, rec, attenuation, scattered) {
    ...
    scattered.copy(new Ray(rec.p, direction, rIn.time));
    return true;
  }
...
export class DiffuseLight extends Material {
  constructor(a) {
    super();
    this.emit = a instanceof Vec3 ? new SolidColor(a) : a;
  }
...
export class Isotropic extends Material {
  ...
  scatter(rIn, rec, attenuation, scattered) {
    scattered.copy(new Ray(rec.p, randomUnitVector(), rIn.time));
    attenuation.copy(this.albedo.value(rec.u, rec.v, rec.p));
    return true;
  }
}
